...
def Algo1(key):
	...
	data = f.decrypt(secret_data) # decrypt the file
	target_file.close()
	return data
...
def decrypter():
	tools.empty_folder('files') # empty the files folder
	key_1 = "" # key_1 is the key for the first algorithm
	list_directory = tools.list_dir('key') # list the files in the key folder
	filename = './key/' + list_directory[0] # get the name of the key file
	public_key = open(filename,"rb") 
	for line in public_key: 
		key_1 = key_1 + line # read the key file
	public_key.close()
	secret_information = Algo1(key_1) # decrypt the file using the key
	list_information = secret_information.split(':::::') # split the information
	...
	files = sorted(tools.list_dir('encrypted')) # list the files in the encrypted folder
